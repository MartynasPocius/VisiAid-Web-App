...
const Directions = () => {
  ...
  const [closestAngle, setClosestAngle] = useState(null);
  ...
  const findAndLogClosestAngle = (userLocation, pathWithAngles) => {
    if (!userLocation || !pathWithAngles || pathWithAngles.length === 0) return;
  
    let closest = null;
    let minDist = Infinity;
  
    pathWithAngles.forEach(item => {
      const dist = Math.hypot(item.coordinate.lat - userLocation.lat, item.coordinate.lng - userLocation.lng);
      if (dist < minDist) {
        minDist = dist;
        closest = item;
      }
    ...
    if (closest) {
      setClosestAngle(~~closest.angle)
      console.log(`Closest angle: ${~~closest.angle} degrees`);
      sendData(~~closest.angle);
    }
  };
    ...
    if (isTrackingActive && navigator.geolocation) {
      watchId = navigator.geolocation.watchPosition(
        ({ coords }) => {
          ...
          findAndLogClosestAngle(userLocation, pathDirections);
          //sendData();
        },
        (error) => {
  ...
  }, [isTrackingActive, pathDirections]);
  ...
  const sendData = async (angle) => {
    if (characteristic && angle) {
      try {
        ...
        setConnectionStatus(`Data sent: ${angle}`);
      } catch (error) {
  ...
  const handleDecline = () => {
    ...
    setClosestAngle(null);
    // Additional state resets if necessary, such as resetting the currentLocation might not be needed if you always want to fetch the current location on app start
